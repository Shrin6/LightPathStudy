...
`;
    ...
    // This ensures ALL documents in the collection are included, not just semantically relevant ones
    if (!collectionContext || collectionContext.length < 300) {
      console.log('Using CHUNKS_FALLBACK to include ALL documents...');
    ...
    if (!collectionContext || collectionContext.length < 300) {
      console.log('Trying PARSED_CONTENT_FALLBACK - combining ALL files...');
      ...
      const contentParts = validFiles.map((f: any) => f.parsed_content);
      collectionContext = contentParts.join('\n\n');
      contextSource = 'PARSED_CONTENT_FALLBACK';
      console.log(`[${contextSource}] Combined ${validFiles.length} files (${files.length} total found), total length: ${collectionContext.length}`);
      console.log(`[${contextSource}] Files included: ${validFiles.map((f: any) => f.file_name).join(', ')}`);
    }
